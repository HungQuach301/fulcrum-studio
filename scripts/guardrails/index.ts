...
export interface DLineage { evidenceRepair?: {rounds:string[];merges:Array<{head:string;base:string;candidate:string;pr:number}>;approval:string;paths:string[]}; evidenceVolume?: {rounds:string[];merges:Array<{head:string;base:string;candidate:string;pr:number}>;approval:string;paths:string[];activations:string[];continuations:Array<{head:string;round:number;sourceRun:number;volumes:number[]}>}; head:string; code:string; candidateBase:string; batch:string|null; origin:string|null; closure:boolean; closureBase?:string; unmerged:string[]; paths:string[]; dataPaths:string[]; epochs:Array<{code:string;base:string;candidate:string;pr:number}>; data:Array<{commit:string;parent:string;operation:string;code:string}>; }
