    ...
    def visit(commit):
        ...
        if commit==P:
            ...
            if phase=="implementation":
                ...
                require(1<=int(field(message,"Fulcrum-Candidate-Round"))<=5,"D-candidate-round")
            else:
...
def runtime(root,head):
    ...
    outcomes=[]
    for number,merge,expected in HISTORY+[(x["pr"],x["code"],x["candidate"]) for x in state["epochs"]]:
        try:
