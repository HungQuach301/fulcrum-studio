...
async function main(){
 ...
 const requestId="2".repeat(64),path="/actions/workflows/ci.yml/dispatches",request={ref:"main",inputs:{request_id:requestId}},response=Buffer.from(JSON.stringify({workflow_run_id:456,run_url:"https://api.github.com/repos/HungQuach301/fulcrum-studio/actions/runs/456",html_url:"https://github.com/HungQuach301/fulcrum-studio/actions/runs/456"}));
 const intentFrames=[frame("dispatch-"+requestId+"-intent.json",Buffer.from(JSON.stringify({method:"POST",path,body:request,requestDigest:digest(JSON.stringify(request))}))),frame("dispatch-"+requestId+"-raw.json",response),frame("dispatch-"+requestId+"-metadata.json",Buffer.from(JSON.stringify({method:"POST",path,status:200,rawEncoding:"identity",bytes:response.length,sha256:digest(response),capped:false,requestDigest:digest(JSON.stringify(request))})))];
